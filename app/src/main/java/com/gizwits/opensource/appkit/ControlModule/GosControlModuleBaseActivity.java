...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 * ===========================================================
	 */
	// 数据点"DeviceOnOff"对应的标识名
	protected static final String KEY_DEVICEONOFF = "DeviceOnOff";
	// 数据点"TempSet"对应的标识名
	protected static final String KEY_TEMPSET = "TempSet";
	// 数据点"TempNow"对应的标识名
	protected static final String KEY_TEMPNOW = "TempNow";
	 ...
	 * ===========================================================
	 */
	// 数据点"TempSet"对应seekbar滚动条补偿值
	protected static final int TEMPSET_OFFSET = 0;
	// 数据点"TempSet"对应数据点增量值
	protected static final int TEMPSET_ADDITION = -20;
	// 数据点"TempSet"对应数据点定义的分辨率
	protected static final int TEMPSET_RATIO = 1;
	 ...
	 * ===========================================================
	 */
	// 数据点"DeviceOnOff"对应的存储数据
	protected static boolean data_DeviceOnOff;
	// 数据点"TempSet"对应的存储数据
	protected static int data_TempSet;
	// 数据点"TempNow"对应的存储数据
	protected static int data_TempNow;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				if (dataKey.equals(KEY_DEVICEONOFF)) {
					data_DeviceOnOff = (Boolean) map.get(dataKey);			
				}
				if (dataKey.equals(KEY_TEMPSET)) {
			
					data_TempSet = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_TEMPNOW)) {
			
					data_TempNow = (Integer) map.get(dataKey);
				}
			}
		}
